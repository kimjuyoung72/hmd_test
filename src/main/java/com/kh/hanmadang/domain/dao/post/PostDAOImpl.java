...
public class PostDAOImpl implements PostDAO{
  ...
  public Long saveOrigin(Post post) {
    ...
    sql.append("insert into post (post_id,bcategory,title,email,nickname,bcontent,bgroup) ");
  ...
  public Long saveReply(Long pbbsId, Post replyBbs) {
    //부모글 참조반영
    Post post = addInfoOfParentToChild(pbbsId,replyBbs);
    ...
    jt.update(new PreparedStatementCreator() {
      ...
      public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
        ...
        pstmt.setString(4, post.getNickname());
        pstmt.setString(5, post.getBcontent());
        pstmt.setLong(6, post.getPbbsId());
        pstmt.setLong(7, post.getBgroup());
        pstmt.setInt(8, post.getStep());
        pstmt.setInt(9, post.getBindent());
        return pstmt;
      }
    },keyHolder);
  ...
  //답글에 부모정보 반영하기
  private Post addInfoOfParentToChild(Long pbbsId, Post replyBbs) {
    //부모글
    Post post = findByBbsId(pbbsId);

    //부모글의 카테고리 가져오기
    replyBbs.setBcategory(post.getBcategory());

    //bgroup 로직
    // 답글의 bgroup = 부모글의 bgroup
    replyBbs.setBgroup(post.getBgroup());

    //step 로직
    //1) 부모글의 bgroup값과 동일한 게시글중 부모글의 step보다큰 게시글의 bstep을 1씩 증가
    int affectedRows = updateBstep(post);
    //2) 답글의 bstep값은 부모글의 bstep값 + 1
    replyBbs.setStep(post.getStep()+1);

    //bindent 로직
    // 답글의 bindent = 부모글의 bindent + 1
    replyBbs.setBindent(post.getBindent()+1);
  ...
  private int updateBstep(Post post) {
    ...
    int affectedRows = jt.update(sql.toString(), post.getBgroup(), post.getStep());
  ...
  public int totalCount(String bcategory) {
    String sql = "select count(*) from post where bcategory = ? ";
