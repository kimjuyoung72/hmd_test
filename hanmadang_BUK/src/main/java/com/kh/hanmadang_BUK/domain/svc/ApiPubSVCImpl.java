...
//            res = xmlMapper.readValue(xmlStr, (new ArrayList<PEvent>()).getClass());
            res = xmlMapper.readValue(xmlStr, new TypeReference<List<PEvent>>(){});
            ...
            for (int i = 0; i < res.size(); i++) {
                ...
                urlBuilder = new StringBuilder("http://www.kopis.or.kr/openApi/restful/pblprfr/"+ res.get(i).getMt20id() + "?service=" + serviceKey );

                url = new URL(urlBuilder.toString());                           // 3. URL 객체 생성
                System.out.println(url);
                conn = (HttpURLConnection) url.openConnection();  // 4. 요청하고자 하는 URL과 통신하기 위한 Connection 객체 생성.
                conn.setRequestMethod("GET");                                       // 5. 통신을 위한 메소드 SET.
                ...
                sb = new StringBuilder();     // 9. 저장된 데이터를 라인별로 읽어 StringBuilder 객체로 저장.
                while ((line = rd.readLine()) != null) {
                    sb.append(line);
                }
                rd.close();                                 // 10. 객체 해제
                conn.disconnect();
                xmlStr = sb.toString();
//                re = xmlMapper.readValue(xmlStr, PEvent.class);
